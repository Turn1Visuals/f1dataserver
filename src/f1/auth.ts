...
export function loadToken(): string | null {
  ...
  try {
    if (existsSync(TOKEN_FILE)) {
      const { token } = JSON.parse(readFileSync(TOKEN_FILE, "utf-8")) as { token: string };
...
export function getTokenStatus(): { loggedIn: boolean; expiresAt: string | null; pending: boolean } {
  const pending = loginPending;
  try {
    const token = loadToken();
    if (!token) return { loggedIn: false, expiresAt: null, pending };
...
export async function startLogin(): Promise<void> {
  if (loginPending) throw new Error("Login already in progress");

  const chromePath = _findChrome();
  if (!chromePath) throw new Error("Google Chrome not found — install Chrome to use F1 login");
...
function _findChrome(): string | null {
  const candidates = [
...
async function _getCookieViaCDP(): Promise<string | null> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => { ws.terminate(); if (!settled) { settled = true; reject(new Error("CDP timeout")); } }, 5000);
    ...
    ws.on("message", (raw) => {
      ...
      try {
        ...
        const cookie = cookies.find(
          (c) => c.name === COOKIE_NAME && c.domain.includes("formula1.com")
        );
        ...
        let parsed: { data?: { subscriptionToken?: string } };
        try {
          ...
          try {
            ...
            console.log("[f1/auth] Could not parse cookie value:", (e2 as Error).message);
            resolve(null);
            return;
          }
        }
        ...
        if (!token) console.log("[f1/auth] No subscriptionToken — data keys:", Object.keys(parsed.data ?? {}));
        resolve(token);
      } catch (e) {
        console.log("[f1/auth] CDP parse error:", (e as Error).message);
        resolve(null);
      }
    });

    ws.on("error", (err) => { clearTimeout(timeout); if (!settled) { settled = true; reject(err); } });
  });
}

function _getExp(token: string): number | null {
  try {
