...
async function reassignOrDelete<T>(
  update: () => Promise<T>,
  remove: () => Promise<T>,
): Promise<void> {
  try {
...
for (const [name, arr] of dupes) {
  ...
  for (const dup of duplicates) {
    await reassignOrDelete(
      () => prisma.result.updateMany({ where: { driverId: dup.id }, data: { driverId: canonical.id } }),
      () => prisma.result.deleteMany({ where: { driverId: dup.id } }),
    );
    await reassignOrDelete(
      () => prisma.qualifyingResult.updateMany({ where: { driverId: dup.id }, data: { driverId: canonical.id } }),
      () => prisma.qualifyingResult.deleteMany({ where: { driverId: dup.id } }),
    );
    await reassignOrDelete(
      () => prisma.driverSeason.updateMany({ where: { driverId: dup.id }, data: { driverId: canonical.id } }),
      () => prisma.driverSeason.deleteMany({ where: { driverId: dup.id } }),
    );
    await reassignOrDelete(
      () => prisma.driverStanding.updateMany({ where: { driverId: dup.id }, data: { driverId: canonical.id } }),
      () => prisma.driverStanding.deleteMany({ where: { driverId: dup.id } }),
    );
    await reassignOrDelete(
      () => prisma.lapTime.updateMany({ where: { driverId: dup.id }, data: { driverId: canonical.id } }),
      () => prisma.lapTime.deleteMany({ where: { driverId: dup.id } }),
    );
    await reassignOrDelete(
      () => prisma.pitStop.updateMany({ where: { driverId: dup.id }, data: { driverId: canonical.id } }),
      () => prisma.pitStop.deleteMany({ where: { driverId: dup.id } }),
    );
