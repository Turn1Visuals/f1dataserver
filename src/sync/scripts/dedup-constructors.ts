...
for (const [name, arr] of dupes) {
  ...
  for (const dup of duplicates) {
    await reassignOrDelete(
      () => prisma.result.updateMany({ where: { constructorId: dup.id }, data: { constructorId: canonical.id } }),
      () => prisma.result.deleteMany({ where: { constructorId: dup.id } }),
    );
    await reassignOrDelete(
      () => prisma.qualifyingResult.updateMany({ where: { constructorId: dup.id }, data: { constructorId: canonical.id } }),
      () => prisma.qualifyingResult.deleteMany({ where: { constructorId: dup.id } }),
    );
    await reassignOrDelete(
      () => prisma.driverSeason.updateMany({ where: { constructorId: dup.id }, data: { constructorId: canonical.id } }),
      () => prisma.driverSeason.deleteMany({ where: { constructorId: dup.id } }),
    );
    await reassignOrDelete(
      () => prisma.constructorStanding.updateMany({ where: { constructorId: dup.id }, data: { constructorId: canonical.id } }),
      () => prisma.constructorStanding.deleteMany({ where: { constructorId: dup.id } }),
    );
