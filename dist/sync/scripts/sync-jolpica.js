...
import { fetchSeasons, fetchDrivers, fetchConstructors, fetchCircuits, fetchRaces, fetchResults, fetchQualifying, fetchSprintResults, fetchDriverStandings, fetchConstructorStandings, fetchLapTimes, fetchPitStops, } from "../sources/jolpica.js";
...
async function syncRaceResults(year, round, sessionId) {
    let raw;
    try {
...
async function syncQualifyingResults(year, round, eventId) {
    let raw;
    try {
...
async function syncSprintResults(year, round, eventId) {
    let raw;
    try {
...
async function upsertQualifyingRows(raw, sessionId, year) {
    for (const r of raw) {
        const driver = await prisma.driver.findUnique({ where: { jolpikaId: r.Driver.driverId } });
        const constructor = await prisma.constructor.findUnique({ where: { jolpikaId: r.Constructor.constructorId } });
        if (!driver || !constructor)
...
async function syncStandings(year, round, sessionId) {
    try {
        ...
        for (const s of driverStandings) {
            ...
            if (!driver || !constructor || isNaN(driverPosition))
                continue;
...
async function syncLapTimes(year, round, sessionId) {
    let raw;
    try {
...
async function syncPitStops(year, round, sessionId) {
    let raw;
    try {
...
async function ensureDriverSeason(driverId, constructorId, seasonYear, driverNumber) {
...
//# sourceMappingURL=sync-jolpica.js.map