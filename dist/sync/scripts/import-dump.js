...
async function batchInsert(label, items, fn) {
    let done = 0;
...
// ─── Import steps ─────────────────────────────────────────────────────────────
async function importSeasons(idMap) {
    console.log("Importing seasons...");
    const rows = readCsv("formula_one_season.csv");
...
async function importDrivers(idMap) {
    console.log("Importing drivers...");
    const rows = readCsv("formula_one_driver.csv");
...
async function importConstructors(idMap) {
    console.log("Importing constructors...");
    const rows = readCsv("formula_one_team.csv");
    for (const row of rows) {
...
async function importCircuits(idMap) {
    console.log("Importing circuits...");
    const rows = readCsv("formula_one_circuit.csv");
    for (const row of rows) {
...
async function importEvents(idMap, seasonIdMap, circuitIdMap) {
    console.log("Importing events...");
    const rows = readCsv("formula_one_round.csv");
    let count = 0;
    for (const row of rows) {
        if (row.is_cancelled === "t")
            continue;
        const seasonYear = seasonIdMap.get(`season:${row.season_id}`);
        const circuitId = circuitIdMap.get(`circuit:${row.circuit_id}`);
        if (!seasonYear || !circuitId)
            continue;
    ...
    for (const row of rows) {
        ...
        if (year)
            seasonRoundCounts.set(year, (seasonRoundCounts.get(year) ?? 0) + 1);
    }
...
async function importSessions(idMap, eventIdMap) {
    console.log("Importing sessions...");
    const rows = readCsv("formula_one_session.csv");
    // For Q1/Q2/Q3, we create one QUALIFYING session per round (not three)
    const qualifyingSessionPerRound = new Map(); // round_id → our session id
    const sprintQualifyingSessionPerRound = new Map();
    let count = 0;
    for (const row of rows) {
        if (row.is_cancelled === "t")
            continue;
...
async function importDriverSeasons(driverIdMap, constructorIdMap, seasonIdMap, teamDriverIdMap) {
    console.log("Importing driver seasons...");
    const rows = readCsv("formula_one_teamdriver.csv");
    let count = 0;
    for (const row of rows) {
        const driverId = driverIdMap.get(`driver:${row.driver_id}`);
        const constructorId = constructorIdMap.get(`constructor:${row.team_id}`);
        const seasonYear = seasonIdMap.get(`season:${row.season_id}`);
        if (!driverId || !constructorId || !seasonYear)
            continue;
...
async function importResults(sessionIdMap, roundEntryMap, dumpSessionTypeMap // dump session id → dump session type
) {
    console.log("Importing session entries (results)...");
    const rows = readCsv("formula_one_sessionentry.csv");
    const raceRows = [];
    const qualRows = [];
    for (const row of rows) {
        ...
        if (!dumpType)
            continue;
        if (dumpType === "R" || dumpType === "SR")
            raceRows.push(row);
        else if (["Q1", "Q2", "Q3", "QB", "QO", "QA", "SQ1", "SQ2", "SQ3"].includes(dumpType))
            qualRows.push(row);
    }
    // Race / Sprint results
...
async function importStandings(sessionIdMap, driverIdMap, constructorIdMap, seasonIdMap) {
    console.log("Importing driver standings...");
    const driverRows = readCsv("formula_one_driverchampionship.csv");
    await batchInsert("driver standings", driverRows, async (batch) => {
        for (const row of batch) {
            if (row.is_eligible !== "t")
                continue;
            const sessionId = sessionIdMap.get(`session:${row.session_id}`);
            const driverId = driverIdMap.get(`driver:${row.driver_id}`);
            const seasonYear = seasonIdMap.get(`season:${row.season_id}`);
            if (!sessionId || !driverId || !seasonYear)
                continue;
    ...
    const constructorRows = readCsv("formula_one_teamchampionship.csv");
    await batchInsert("constructor standings", constructorRows, async (batch) => {
        for (const row of batch) {
            ...
            const constructorId = constructorIdMap.get(`constructor:${row.team_id}`);
            const seasonYear = seasonIdMap.get(`season:${row.season_id}`);
            if (!sessionId || !constructorId || !seasonYear)
                continue;
...
async function importLapTimes(sessionEntryToDriver) {
    console.log("Importing lap times...");
    const rows = readCsv("formula_one_lap.csv");
...
async function importPitStops(sessionEntryToDriver, lapIdToNumber) {
    console.log("Importing pit stops...");
    const rows = readCsv("formula_one_pitstop.csv");
...
async function main() {
    ...
    const seasonIdMap = new Map(); // "season:N" → year
    const driverIdMap = new Map(); // "driver:N" → cuid
    const constructorIdMap = new Map(); // "constructor:N" → cuid
    const circuitIdMap = new Map(); // "circuit:N" → cuid
    const eventIdMap = new Map(); // "round:N" → cuid
    const sessionIdMap = new Map(); // "session:N" → cuid
    const teamDriverIdMap = new Map();
    ...
    const roundEntryRows = readCsv("formula_one_roundentry.csv");
    const roundEntryMap = new Map();
    for (const row of roundEntryRows) {
        const td = teamDriverIdMap.get(row.team_driver_id);
        if (td)
            roundEntryMap.set(row.id, { ...td, carNumber: Number(row.car_number) });
    }
    console.log(`  ✓ ${roundEntryMap.size} round entries`);
    // Dump session type map: dump session id → type code
    const dumpSessionRows = readCsv("formula_one_session.csv");
    const dumpSessionTypeMap = new Map();
    for (const row of dumpSessionRows)
        dumpSessionTypeMap.set(row.id, row.type);
    // Session entry → driver/session mapping (for laps + pit stops)
    console.log("Loading session entries index...");
    const sessionEntryRows = readCsv("formula_one_sessionentry.csv");
    const sessionEntryToDriver = new Map();
    for (const row of sessionEntryRows) {
        const entry = roundEntryMap.get(row.round_entry_id);
        const sessionId = sessionIdMap.get(`session:${row.session_id}`);
        if (entry && sessionId)
            sessionEntryToDriver.set(row.id, { driverId: entry.driverId, sessionId });
    }
    ...
    const lapRows = readCsv("formula_one_lap.csv");
    const lapIdToNumber = new Map();
    for (const row of lapRows) {
        if (row.is_deleted !== "t" && row.number)
            lapIdToNumber.set(row.id, Number(row.number));
    }
...
//# sourceMappingURL=import-dump.js.map